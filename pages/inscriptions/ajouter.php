...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // Vérifier doublon
        $chk = $pdo->prepare("SELECT COUNT(*) FROM inscription WHERE id_eleve=? AND id_classe=?");
        $chk->execute([$id_eleve, $id_classe]);
        if ($chk->fetchColumn() > 0) {
            $errors[] = "Cet élève est déjà inscrit dans cette classe.";
        } else {
            $stmt = $pdo->prepare("INSERT INTO inscription (date_inscription, id_eleve, id_classe) VALUES (?, ?, ?)");
            $stmt->execute([$date_insc, $id_eleve, $id_classe]);
...
<div class="container" style="max-width: 600px;">
    <div class="d-flex align-items-center mb-3">
        ...
        <h4 class="mb-0 fw-bold text-success"><i class="bi bi-journal-plus me-2"></i>Ajouter une Inscription</h4>
    </div>
        ...
        <div class="card-body">
            <form method="POST" action="ajouter.php">
                    ...
                    <select name="id_eleve" class="form-select" required>
                        ...
                        <?php foreach ($eleves as $e): ?>
                            <option value="<?= $e['id_eleve'] ?>"
                                <?= (isset($_POST['id_eleve']) && $_POST['id_eleve'] == $e['id_eleve']) ? 'selected' : '' ?>>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-success"><i class="bi bi-save me-1"></i>Inscrire</button>
