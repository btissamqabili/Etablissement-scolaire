...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE eleve SET nom=?, prenom=?, date_naissance=?, email=? WHERE id_eleve=?");
        $stmt->execute([$nom, $prenom, $date_n, $email ?: null, $id]);
        header("Location: liste.php?msg=modifie");
        exit;
    }

    // Garder les valeurs POST en cas d'erreur
    $eleve = array_merge($eleve, compact('nom','prenom','date_n','email'));
    $eleve['date_naissance'] = $date_n;
}
?>
<!DOCTYPE html>
<html lang="fr">
...
<div class="container" style="max-width: 600px;">
    <div class="d-flex align-items-center mb-3">
        ...
        <h4 class="mb-0 fw-bold text-warning"><i class="bi bi-pencil-square me-2"></i>Modifier l'Élève</h4>
    </div>
        ...
        <div class="card-body">
            <form method="POST" action="modifier.php?id=<?= $id ?>">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="date_naissance" class="form-control" value="<?= htmlspecialchars($eleve['date_naissance']) ?>" required>
                </div>
                <div class="mb-4">
                    <label class="form-label fw-semibold">Email</label>
                    <input type="email" name="email" class="form-control" value="<?= htmlspecialchars($eleve['email'] ?? '') ?>">
                </div>
